  ...
  booksGrid.innerHTML = visibleBooks
    .map((book, index) => {
      ...
      const titleLengthClass =
        title.length > 72
          ? " book-card__title--extra-long"
          : title.length > 44
            ? " book-card__title--long"
            : "";
      const color =
        categoryColors[book.category] || categoryColors[category] || "#6d4c41";
      const actionLabel = locale.readSummary;
      const bookStatus = progress[book.slug];
      const statusLabel = bookStatus === "read" ? locale.statusRead : bookStatus === "reading" ? locale.statusReading : "";
      ...
      return `
        <article class="book-card" data-slug="${book.slug}">
          <a class="book-card__link" href="#${routeBase(language)}${book.slug}" aria-label="${escapeHtml(locale.openSummary + title)}">
            <div class="light"></div>
            ${statusLabel ? `<div class="book-card__status badge badge--${bookStatus}">${escapeHtml(statusLabel)}</div>` : ""}
            <div class="book-card__cover" style="background: radial-gradient(120% 80% at 0% 0%, ${color} 0%, transparent 55%), linear-gradient(180deg, oklch(0.22 0.02 60) 0%, oklch(0.14 0.02 60) 100%);">
              ...
              <div class="book-card__headline">
                ...
                <h2 class="${titleLengthClass.trim()}">${escapeHtml(title)}</h2>
...
export async function renderDetail(
  ...
  const relatedMarkup = related.length
    ...
    : "";
  const isRead = progress[book.slug] === "read";
  ...
  detailViewEl.innerHTML = `
    <article class="detail-page">
      ...
      ${!isRead ? `
        <section class="detail-page__actions">
          <button type="button" class="mark-as-read-button" data-mark-as-read="${book.slug}">
            ${escapeHtml(locale.markAsRead)}
          </button>
        </section>
      ` : ''}
