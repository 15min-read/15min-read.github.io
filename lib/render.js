...
export function renderFilters(books, state, filtersEl, updateLayoutOffsets) {
  ...
  const availableCategories = [
    ...
    ...new Set(
      books.map((book) => localizedField(book, "category", language)),
    ),
  ];
...
export function renderBooks(
  ...
  if (categoryCountEl) {
    categoryCountEl.textContent = String(
      new Set(books.map((book) => localizedField(book, "category", language))).size,
    );
  }
  ...
  booksGrid.innerHTML = visibleBooks
    .map((book, index) => {
      ...
      return `
        <a class="book-card" href="#${routeBase(language)}${book.slug}" data-slug="${book.slug}" aria-label="${escapeHtml(
          state.language === "en"
            ? `Open summary: ${title}`
            : `Abrir resumo: ${title}`,
        )}">
...
export async function renderDetail(
  ...
  const detailLabels = {
    ...
    summaryHeading:
      activeLanguage === "en" ? "Interpretive Summary" : "Resumo interpretativo",
    sidebarHeading:
      activeLanguage === "en"
  ...
  const relatedMarkup = related.length
    ? `<section class="detail-page__related">
         ...
         <div class="related-grid">
           ${related
             .map((entry) => {
               const relatedTitle = localizedField(entry, "title", activeLanguage);
               const relatedHook = localizedField(entry, "hook", activeLanguage);
               return `
