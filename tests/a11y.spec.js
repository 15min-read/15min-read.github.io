...
test("home page has no detectable accessibility violations", async ({
  ...
  if (accessibilityScan.violations.length) {
    throw new Error(
      `A11y: ${accessibilityScan.violations.length} violations found`,
    );
  }
});

test("detail page has no detectable accessibility violations", async ({
  page,
}) => {
  await page.goto("http://localhost:4173/#/livros/habitos-atomicos", {
