...
test("live site - localized routes and detail pages", async ({ page }) => {
  // Portuguese catalog
  await page.goto("https://15min-read.github.io/#/livros", {
  ...
  await page.goto("https://15min-read.github.io/#/books", {
    waitUntil: "domcontentloaded",
  });
  ...
  await page.goto("https://15min-read.github.io/#/books/habitos-atomicos", {
    waitUntil: "domcontentloaded",
  });
  await page.evaluate(() => localStorage.setItem("siteLanguage", "en"));
  await page.reload({ waitUntil: "networkidle" });
  await page.waitForSelector(".detail-page__header h1");
  await expect(page.locator(".detail-page__header h1")).toContainText(
    "Atomic Habits",
  );
});
